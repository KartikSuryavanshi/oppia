...
describe('Url Service', () => {
  ...
  let mockLocation: Pick<
    Location,
    'href' | 'origin' | 'pathname' | 'hash' | 'search'
  >;
  ...
  it('should return correct query value list for each query field', () => {
    ...
    expect(urlService.getQueryFieldValuesAsList('field2')).toEqual(
      expectedList2
    );
  });
  ...
  it('should correctly encode and add query field and value to url', () => {
    ...
    expect(urlService.addField(baseUrl, queryField, queryValue)).toBe(
      expectedUrl1
    );
  ...
  it('should correctly retrieve story url fragment from url', () => {
    ...
    expect(urlService.getStoryUrlFragmentFromLearnerUrl()).toBe('story-one');
    mockLocation.search =
  ...
  it('should correctly retrieve topic url fragment from url', () => {
    ...
    expect(urlService.getTopicUrlFragmentFromLearnerUrl()).toBe('topic-name');
    mockLocation.pathname = '/learn/math/topic-name/practice';
    expect(urlService.getTopicUrlFragmentFromLearnerUrl()).toBe('topic-name');
    mockLocation.pathname = '/explore/16';
    mockLocation.search = '?topic_url_fragment=topic';
    expect(urlService.getTopicUrlFragmentFromLearnerUrl()).toBe('topic');
    mockLocation.pathname = '/topc/abcdefgijklm';
    expect(function () {
  ...
  it('should correctly retrieve classroom name from url', () => {
    mockLocation.pathname = '/learn/math/abcdefgijklm';
    expect(urlService.getClassroomUrlFragmentFromLearnerUrl()).toBe('math');
    mockLocation.pathname = '/explore/16';
    mockLocation.search = '&classroom_url_fragment=math';
    expect(urlService.getClassroomUrlFragmentFromLearnerUrl()).toBe('math');
  ...
  it('should correctly construct learner classroom and story urls', () => {
    ...
    expect(function () {
      urlService.getLearnerClassroomUrl();
    }).toThrowError('Invalid URL for classroom');
    expect(function () {
      urlService.getLearnerTopicStoryUrl();
    }).toThrowError('Invalid URL for classroom');
    expect(function () {
      urlService.getLearnerTopicStudyGuideUrl();
    }).toThrowError('Invalid URL for classroom');
  });
  ...
  it('should correctly retrieve classroom url fragment from url', () => {
    ...
    expect(urlService.getClassroomUrlFragmentFromUrl()).toBe('class name');
  ...
  it('should correctly retrieve skill id from url', () => {
    ...
    expect(urlService.getSkillIdFromUrl()).toBe('abcdefghijkl');
  ...
  it('should correctly retrieve collection id from url in exploration player', function () {
    mockLocation.search = '?collection_id=abcdefghijkl';
    expect(urlService.getCollectionIdFromExplorationUrl()).toBe('abcdefghijkl');

    mockLocation.search = '?collection=abcdefghijkl';
    expect(urlService.getCollectionIdFromExplorationUrl()).toBe(null);
